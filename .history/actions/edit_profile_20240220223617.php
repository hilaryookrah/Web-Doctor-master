...
if($res->num_rows>0){
    $row=$res->fetch_assoc();
    if(password_verify(,$row['passwd'])){
        $query="UPDATE people SET fname=?,lname=?,email=?,tel=?,address=? WHERE email=?";
        $stmt=$conn->prepare($query);
        $stmt->bind_param("ssssss",$fname,$lname,$email,$phone,$address,$or_email);
        $stmt->execute();
